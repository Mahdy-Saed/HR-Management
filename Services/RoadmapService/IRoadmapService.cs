...
namespace HR_Carrer.Services.RoadmapService
{
    ...
    public class RoadmapService : IRoadmapService
    {
        ...
        public RoadmapService(IEmployeeRepo employeeRepo, IRoadmapRepo roadmapRepo, IMapper mapper, IRequestRepo requestRepo)
        {
        ...
        public async Task<ServiceResponce<RoadmapResponceDto>> CreateRoadmap(Guid id, RoadmapRequestDto roadmapRequestDto)
        {
            ...
            if (roadmapRequestDto is null) return ServiceResponce<RoadmapResponceDto>.Fail("Must enter information", 400);
            
              var requestOfEmployee = await _requestRepo.promotionRequest(id);

            if (requestOfEmployee == null) return ServiceResponce<RoadmapResponceDto>.Fail("The employee Did Not send a request for promotion.", 400);
